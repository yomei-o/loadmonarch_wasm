...
sh tools/lowpri.sh "$GCC" -O2 -Wall -Isrc -o tests/unit_window.exe    tests/unit_window.c src/world.c src/host.c src/zip.c src/bz.c src/gfx.c
echo "built tests/unit_window.exe"
sh tools/lowpri.sh "$GCC" -O2 -Wall -Isrc -o tests/progress_window.exe tests/progress_window.c src/world.c src/host.c src/zip.c src/bz.c src/gfx.c
...
sh tools/lowpri.sh "$GCC" -O2 -Wall -Isrc -o tests/midi_test.exe       tests/midi_test.c src/midi.c src/synth.c src/smf.c src/host.c src/zip.c
echo "built tests/midi_test.exe"
sh tools/lowpri.sh "$GCC" -O2 -Wall -Isrc -o tests/dialog_test.exe     tests/dialog_test.c $SIM
...
sh tools/lowpri.sh "$GCC" -O2 -Wall -Isrc -o tests/frame_dump.exe      tests/frame_dump.c $SIM
